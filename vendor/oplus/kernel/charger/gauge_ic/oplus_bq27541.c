...
int bq27541_get_batt_manu_date_by_alt_manufacturer_access(void)
{
	...
	struct gauge_track_info_reg extend = { BQ27Z561_SUBCMD_MANU_DATE, BQ27Z561_SUBCMD_MANU_DATE_READ_BUF_LEN };
...
try:
	...
	if (ret < 0)
		goto error;
	usleep_range(1000, 1000);
	ret = gauge_read_i2c_block(gauge_ic, BQ27Z561_DATAFLASHBLOCK, (extend.len + 2), extend_data);
	if (ret < 0)
		goto error;
...
RETRY:
	while (retry_id < BQ27Z561_BATT_INFO_RETRY_MAX) {
		...
		mutex_lock(&gauge_ic->gauge_alt_manufacturer_access);
		ret = gauge_i2c_txsubcmd(gauge_ic, BQ27Z561_DATAFLASHBLOCK, BQ27Z561_BATTINFO_VDMDATA_CMD);
		if (ret < 0)
			goto error;
		usleep_range(1000, 1000);
		ret = gauge_read_i2c_block(gauge_ic, BQ27Z561_DATAFLASHBLOCK, BQ27Z561_BATT_VDM_DATA_READ_BUF_LEN, buf);
		if (ret < 0)
			goto error;
		...
		if ((buf[0] == (BQ27Z561_BATTINFO_VDMDATA_CMD & 0xFF))
				...
				date_temp = buf[BQ27Z561_BATT_FIRST_USAGE_DATE_L] | buf[BQ27Z561_BATT_FIRST_USAGE_DATE_H] << 8;
				ui_soh = buf[BQ27Z561_BATT_UI_SOH];
				ui_cycle_count = buf[BQ27Z561_BATT_UI_CYCLE_COUNT_L] | buf[BQ27Z561_BATT_UI_CYCLE_COUNT_H] << 8;
	...
	if (retry_id < BQ27Z561_BATT_INFO_RETRY_MAX) {
		if (buf[BQ27Z561_BATT_FIRST_USAGE_DATE_CHECK] != BQ27Z561_BATTINFO_NO_CHECKSUM) {
			check_sum = (BQ27Z561_BATTINFO_DEFAULT_CHECKSUM - buf[BQ27Z561_BATT_FIRST_USAGE_DATE_L]
						- buf[BQ27Z561_BATT_FIRST_USAGE_DATE_H]) & 0xFF;
			if (check_sum != buf[BQ27Z561_BATT_FIRST_USAGE_DATE_CHECK]) {
				pr_err("bq28z610_get_batt_vdm_data fud_check fail calc[0x%02x] read[0x%02x], retry.",
		...
		if (buf[BQ27Z561_BATT_UI_SOH_CHECK] != BQ27Z561_BATTINFO_NO_CHECKSUM) {
			check_sum = (BQ27Z561_BATTINFO_DEFAULT_CHECKSUM - buf[BQ27Z561_BATT_UI_SOH]) & 0xFF;
			if (check_sum != buf[BQ27Z561_BATT_UI_SOH_CHECK]) {
				pr_err("bq28z610_get_batt_vdm_data uisoh_check fail calc[0x%02x] read[0x%02x], retry.",
		...
		if (buf[BQ27Z561_BATT_UI_CYCLE_COUNT_CHECK] != BQ27Z561_BATTINFO_NO_CHECKSUM) {
			check_sum = (BQ27Z561_BATTINFO_DEFAULT_CHECKSUM - buf[BQ27Z561_BATT_UI_CYCLE_COUNT_L]
						- buf[BQ27Z561_BATT_UI_CYCLE_COUNT_H]) & 0xFF;
			if (check_sum != buf[BQ27Z561_BATT_UI_CYCLE_COUNT_CHECK]) {
				pr_err("bq28z610_get_batt_vdm_data uicc_check fail calc[0x%02x] read[0x%02x], retry.",
		...
		if (buf[BQ27Z561_BATT_USED_FLAG_CHECK] != BQ27Z561_BATTINFO_NO_CHECKSUM) {
			check_sum = (BQ27Z561_BATTINFO_DEFAULT_CHECKSUM - buf[BQ27Z561_BATT_USED_FLAG]) & 0xFF;
			if (check_sum != buf[BQ27Z561_BATT_USED_FLAG_CHECK]) {
				pr_err("bq28z610_get_batt_vdm_data used_flag fail calc[0x%02x] read[0x%02x], retry.",
...
static void oplus_get_manu_battinfo_work(struct work_struct *work)
{
	...
	if (ret < 0) {
		pr_err("oplus_bq27541_get_batt_manufact_info get manu-date failed");
	}
	ret = bq27541_get_batt_manu_info_by_alt_manufacturer_access();
	if (ret < 0) {
		pr_err("oplus_bq27541_get_batt_manufact_info get manu_info failed");
	}
...
int bq27541_get_batt_first_usage_date(char *info, int len)
{
	...
	schedule_delayed_work(&gauge_ic->get_manu_battinfo_work, round_jiffies_relative(msecs_to_jiffies(100)));

	msleep(300);
	date_len = snprintf(info, len, "%d-%02d-%02d", (((gauge_ic->first_usage_date >> 9) & 0x7F) + 1980),
			(gauge_ic->first_usage_date >> 5) & 0xF, gauge_ic->first_usage_date & 0x1F);
...
int bq27541_set_batt_first_usage_date(const char *info)
{
	...
	u8 check_sum = 0x00;
	u8 write_data[BQ27Z561_BATT_FIRST_USAGE_DATE_WLEN] = {BQ27Z561_SUBCMD_FIRST_USAGE_DATE_WADDR & 0xFF,
					BQ27Z561_SUBCMD_FIRST_USAGE_DATE_WADDR >> 8, 0x00, 0x00, 0x00};
	u8 check_data[2] = {BQ27Z561_BATTINFO_DEFAULT_CHECKSUM, BQ27Z561_BATT_FIRST_USAGE_DATE_WLEN + 2};
	int i = 0;
	int ret = 0;
	int year = 0;
	int month = 0;
	...
	sscanf(info, "%d-%d-%d", &year, &month, &day);
	first_usage_date = (((year - 1980) & 0x7F) << 9) | ((month & 0xF) << 5) | (day & 0x1F);
	check_sum = 0xFF - ((first_usage_date >> 8) & 0xFF) - (first_usage_date & 0xFF);
	chg_info("first_usage_date:%d-%d-%d", year, month, day);
	chg_info("first_usage_date data=0x%04x", first_usage_date);
	...
	if (!bq27z561_deep_init()) {
		pr_err("bq28z610 deep init failed");
		return -1;
	}
	ret = gauge_write_i2c_block(gauge_ic, BQ27Z561_DATAFLASHBLOCK, BQ27Z561_BATT_FIRST_USAGE_DATE_WLEN, write_data);
	if (ret < 0) {
		return -1;
	}
	usleep_range(1000, 1000);
...
int bq27z561_get_bq27z561_seal_flag(void)
{
	...
	pr_err("bq27z561_get_bq27z561_seal_flag = %d\n", gauge_ic->bq27z561_seal_flag);
...
int bq27z561_set_bq27z561_seal_flag(int seal_flag)
{
	...
	pr_err("bq27z561_set_bq27z561_seal_flag = %d\n", gauge_ic->bq27z561_seal_flag);
	return 0;
}
...
int bq27541_set_batt_ui_cc(int ui_cycle_count)
{
	...
	chg_info("ui_cycle_count_data=0x%04x", ui_cycle_count_data);
	...
	ret = gauge_write_i2c_block(gauge_ic, BQ27Z561_DATAFLASHBLOCK, BQ27Z561_BATT_UI_CC_WLEN, write_data);
	if (ret < 0) {
		return -1;
	}
	usleep_range(1000, 1000);
...
int bq27541_set_batt_ui_soh(int ui_soh)
{
	...
	chg_info("set_batt_ui_soh data=0x%04x", ui_soh_data);
	...
	ret = gauge_write_i2c_block(gauge_ic, BQ27Z561_DATAFLASHBLOCK, BQ27Z561_BATT_UI_SOH_WLEN, write_data);
	if (ret < 0) {
		return -1;
	}
	usleep_range(1000, 1000);
...
int bq27541_set_batt_used_flag(int used_flag)
{
	...
	chg_info("set_batt_used_flag data=0x%04x", used_flag_data);
	...
	ret = gauge_write_i2c_block(gauge_ic, BQ27Z561_DATAFLASHBLOCK, BQ27Z561_BATT_USED_FLAG_WLEN, write_data);
	if (ret < 0) {
		return -1;
	}
	usleep_range(1000, 1000);
...
int oplus_bq27541_get_battinfo_sn(char buf[], int len)
{
	...
	chg_info("BattSN:%s", chip->battinfo.batt_serial_num);
...
static struct oplus_gauge_operations bq27541_gauge_ops = {
	...
	.get_bq27z561_seal_flag = bq27z561_get_bq27z561_seal_flag,
	.set_bq27z561_seal_flag = bq27z561_set_bq27z561_seal_flag,
...
static int bq27z561_unseal(struct chip_bq27541 *chip, u32 key)
{
	...
	do {
		...
		gauge_write_i2c_block(chip, 0x00, 2, seal_cmd_1);
		pr_err("%s write {0x14,0x04} --> 0x00\n", __func__);
		usleep_range(1000000, 1000000);

		gauge_write_i2c_block(chip, 0x00, 2, seal_cmd_2);
		pr_err("%s write {0x72,0x36} --> 0x00\n", __func__);
		usleep_range(10000, 10000);
	...
	pr_err("%s bq27z561_unseal flag [%d]\n", __func__, rc);
	if (rc < 0) {
		oplus_chg_gauge_seal_unseal_fail(OPLUS_GAUGE_UNSEAL_FAIL);
	}
	return rc;
...
rerun:
	...
	schedule_delayed_work(&fg_ic->get_manu_battinfo_work, round_jiffies_relative(msecs_to_jiffies(100)));
