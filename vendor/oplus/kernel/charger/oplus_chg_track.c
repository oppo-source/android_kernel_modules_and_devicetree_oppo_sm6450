...
static void oplus_chg_track_rechg_info_trigger_work(struct work_struct *work)
{
	...
	index += snprintf(&(chip->rechg_info_trigger->crux_info[index]), OPLUS_CHG_TRACK_CURX_INFO_LEN - index,
			"$$rechging@@%d$$rechg_soc_en@@%d$$rechg_soc@@%d$$uisoc@@%d$$soc@@%d$$vbatt_max@@%d$$charge_status@@%d",
			track_status->in_rechging, track_status->track_rechg_soc_en, track_status->track_rechg_soc, track_status->debug_soc,
			track_status->debug_uisoc, track_status->batt_max_vol, track_status->prop_status);
