			      ...
			      char *buf)
{
	...
	ret = oplus_gauge_get_battinfo_sn(batt_sn, sizeof(batt_sn));
	if (ret < 0)
...
static ssize_t debug_battery_sn_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t debug_battery_sn_store(struct device *dev,
			      ...
			      char *buf)
{
	...
	ret = oplus_gauge_get_battinfo_manu_date(batt_date, sizeof(batt_date));
	if (ret < 0)
...
static ssize_t battery_bq27z561_seal_flag_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	int bq27z561_seal_flag = 0;
	int len = 0;

	bq27z561_seal_flag = oplus_gauge_get_bq27z561_seal_flag();
	if (bq27z561_seal_flag < 0)
...
static ssize_t  battery_bq27z561_seal_flag_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
	int ret = 0;
	...
	ret = oplus_gauge_set_bq27z561_seal_flag(bq27z561_seal_flag);
	if (ret < 0)
		chg_err("set battery ui cycle count error");

	return count;
}
static DEVICE_ATTR_RW(battery_bq27z561_seal_flag);
			      ...
			      char *buf)
{
	...
	ret = oplus_gauge_get_battinfo_first_usage_date(batt_date, sizeof(batt_date));
	if (ret < 0)
			       ...
			       size_t count)
{
	...
	ret = oplus_gauge_set_battinfo_first_usage_date(buf);
	if (ret < 0)
...
static ssize_t  battery_debug_cc_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count)
{
	int ui_cycle_count;
...
static struct device_attribute *oplus_battery_attributes[] = {
	...
	&dev_attr_battery_bq27z561_seal_flag,
	&dev_attr_gauge_info,
