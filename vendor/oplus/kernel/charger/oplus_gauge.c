...
int oplus_gauge_get_battinfo_manu_date(char *info, int len)
{
	if (!g_gauge_chip)
		return -1;
	else {
...
int oplus_gauge_get_battinfo_first_usage_date(char *info, int len)
{
	if (!g_gauge_chip)
		return -1;
	else {
...
int oplus_gauge_set_battinfo_first_usage_date(const char *info)
{
	if (!g_gauge_chip)
		return -1;
	else {
...
int oplus_gauge_get_bq27z561_seal_flag(void)
{
	if (!g_gauge_chip)
		return -1;
	else {
		if (g_gauge_chip->gauge_ops && g_gauge_chip->gauge_ops->get_bq27z561_seal_flag) {
			return g_gauge_chip->gauge_ops->get_bq27z561_seal_flag();
		}
		return 0;
	}
}

int oplus_gauge_set_bq27z561_seal_flag(int seal_flag)
{
	if (!g_gauge_chip)
		return -1;
	else {
		if (g_gauge_chip->gauge_ops && g_gauge_chip->gauge_ops->set_bq27z561_seal_flag) {
			return g_gauge_chip->gauge_ops->set_bq27z561_seal_flag(seal_flag);
		}
		return 0;
	}
}
...
int oplus_gauge_get_battinfo_sn(char *sn_buff, int size_buffer)
{
	int rc = 0;
	int len = 0;
