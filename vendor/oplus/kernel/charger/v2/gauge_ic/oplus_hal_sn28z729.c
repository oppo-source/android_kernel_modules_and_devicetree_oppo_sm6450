								...
								int *firm_version)
{
	...
	*firm_version = ((cntl1_val[4] << 8) + cntl1_val[5]) & 0xffff;
	chip->firm_ver = ((cntl1_val[4] << 8) + cntl1_val[5]) & 0xffff;
	build_num = ((cntl1_val[6] << 8) + cntl1_val[7]) & 0xffff;
	mutex_unlock(&chip->extended_cmd_access);

	chg_info("firm_version = 0x%4x, build_num = 0x%4x\n", *firm_version, build_num);
