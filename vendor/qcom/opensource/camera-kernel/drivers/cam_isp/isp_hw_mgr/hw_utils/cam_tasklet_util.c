	...
	CAM_IRQ_HANDLER_BOTTOM_HALF        bottom_half_handler)
{
	...
	if (atomic_read(&tasklet->tasklet_active)) {
		...
		spin_unlock_irqrestore(&tasklet->tasklet_lock, flags);
		if (!atomic_read((&tasklet->tasklet.count)))
			tasklet_hi_schedule(&tasklet->tasklet);
		else
			CAM_ERR_RATE_LIMIT(CAM_ISP, "Skip tasklet schedule as it is disabled");
	}
	else
		CAM_ERR_RATE_LIMIT(CAM_ISP, "Tasklet is not active idx:%d", tasklet->index);
}
...
void cam_tasklet_stop(void  *tasklet_info)
{
	...
	tasklet_kill(&tasklet->tasklet);
	tasklet_disable(&tasklet->tasklet);
	tasklet->tasklet_stop_ts = ktime_get();
	cam_tasklet_flush(tasklet);
	CAM_INFO(CAM_ISP, "idx = %d ts = %ld", tasklet->index, tasklet->tasklet_stop_ts);
}
