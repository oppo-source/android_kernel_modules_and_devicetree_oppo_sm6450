	...
	struct cam_eeprom_memory_block_t *block)
{
	...
	for (j = 0; j < block->num_map; j++) {
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		if ((eb_info->eeprom_name != NULL) && (!strcmp(eb_info->eeprom_name, "sc820cs_wide"))) {
			...
			if (rc == 0){
				CAM_ERR(CAM_EEPROM, "sc820cs otp read success");
				return rc;
			}
		}
		else if (e_ctrl->io_master_info.cci_client->sid==0x24) {
		...
		} else if (soc_private->i2c_info.slave_addr == 0x40){
			if(!eeprom_memory_map_read_data(eb_info->i2c_info.slave_addr,emap[j],e_ctrl,memptr)){
				CAM_DBG(CAM_EEPROM, "read otp data successfully");
			} else {
				CAM_ERR(CAM_EEPROM, "cam_eeprom_read_memory_oem failed");
			}
		}else{
#endif
