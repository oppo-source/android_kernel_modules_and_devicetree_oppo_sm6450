	...
	if (probe_ver == CAM_SENSOR_PACKET_OPCODE_SENSOR_PROBE) {
		...
		s_ctrl->sensordata->slave_info.sec_sensor_id =
			sensor_probe_info_v2->reserved[0];
                //unique sensorid info for cloud eeprom
                s_ctrl->sensordata->slave_info.sensor_unique_id_reg_addr =
                        sensor_probe_info_v2->reserved[1];
                s_ctrl->sensordata->slave_info.sensor_unique_id_reg_len=
                        sensor_probe_info_v2->reserved[2];
                s_ctrl->sensordata->slave_info.sensor_unique_id_reg_step =
                        sensor_probe_info_v2->reserved[3];
#endif
	...
	void *arg)
{
    ...
    case CAM_OEM_GET_UNIQUE_SENSORID: {
        mutex_lock(&(s_ctrl->sensor_power_state_mutex));
		 CAM_INFO(CAM_SENSOR,
                    "--Cloud Eeprom--Power up Start for %s sensor_id: 0x%x, slave_addr: 0x%x",
                    s_ctrl->sensor_name,
                    s_ctrl->sensordata->slave_info.sensor_id,
                    s_ctrl->sensordata->slave_info.sensor_slave_addr
                    );
        if(s_ctrl->sensor_power_state == CAM_SENSOR_POWER_OFF)
        {
            ...
            {
                CAM_INFO(CAM_SENSOR,
                    "--Cloud Eeprom--Power up failed for %s sensor_id: 0x%x, slave_addr: 0x%x",
                    s_ctrl->sensor_name,
                    s_ctrl->sensordata->slave_info.sensor_id,
                    s_ctrl->sensordata->slave_info.sensor_slave_addr
                    );
            ...
            {
                CAM_ERR(CAM_SENSOR,
                    "--Cloud Eeprom--Power up success for %s sensor_id: 0x%x, slave_addr: 0x%x",
                    s_ctrl->sensor_name,
                    s_ctrl->sensordata->slave_info.sensor_id,
                    s_ctrl->sensordata->slave_info.sensor_slave_addr
                    );
        ...
        {
            CAM_ERR(CAM_SENSOR, "--Cloud Eeprom--sensor have power up!");
        }
        mutex_unlock(&(s_ctrl->sensor_power_state_mutex));

        rc = oplus_cam_get_unique_sensorid(s_ctrl, arg);
        if (rc < 0)
        {
            CAM_ERR(CAM_SENSOR, "--Cloud Eeprom--CAM_OEM_GET_UNIQUE_SENSORID failed");
            cam_sensor_power_down(s_ctrl);
            goto free_power_settings;
        }
        ...
        if(s_ctrl->sensor_power_state == CAM_SENSOR_POWER_ON)
        {
            ...
            if(rc < 0)
            {
                CAM_ERR(CAM_SENSOR, "--Cloud Eeprom--Fail in %s sensor Power Down",
                        s_ctrl->sensor_name);
                mutex_unlock(&(s_ctrl->sensor_power_state_mutex));
                goto free_power_settings;
            }
            ...
            {
                CAM_INFO(CAM_SENSOR, "--Cloud Eeprom--success in %s sensor Power Down",
                    s_ctrl->sensor_name);
                s_ctrl->sensor_power_state = CAM_SENSOR_POWER_OFF;
            }
        }
        ...
        {
            CAM_ERR(CAM_SENSOR, "--Cloud Eeprom--sensor have power down!");
        }
        mutex_unlock(&(s_ctrl->sensor_power_state_mutex));
