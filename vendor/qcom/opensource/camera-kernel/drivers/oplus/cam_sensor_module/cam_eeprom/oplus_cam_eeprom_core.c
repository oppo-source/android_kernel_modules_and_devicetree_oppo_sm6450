...
#include "sm7435_hi846w_eeprom.h"
...
int eeprom_process_para_before_read(uint32_t slave_addr,struct eeprom_memory_map_init_write_params **pWriteParams,struct OplusOtpCheckInfo **pOtpParams,uint32_t *count_read){
	int eeprom_result = 0;
	if(sm7435_hi846w_i2c_slave_addr == slave_addr){
		CAM_DBG(CAM_EEPROM, "hi846 get otp init paras");
		*pWriteParams = &sm7435_hi846w_eeprom;
		*pOtpParams = &sm7435_hi846w_otp_checkinfo;
		*count_read = (uint32_t)SM7435_HI846W_OTP_DATA_LEN;
	}else{
		eeprom_result = -1;
...
int eeprom_process_para_after_read(uint32_t slave_addr,struct eeprom_memory_map_init_write_params **pWriteParams){
	...
	if(sm7435_hi846w_i2c_slave_addr == slave_addr){
		...
		*pWriteParams = &sm7435_hi846w_eeprom_after_read;
	}else{
		eeprom_result = -1;
		CAM_DBG(CAM_EEPROM, "it is not a valid otp");
	}
...
int eeprom_memory_select_group(struct OplusOtpCheckInfo *pOtpParams){
	...
	if (pOtpParams->groupInfo.IsAvailable) {
		for( j=0; j < GROUPNUMMAX; j++) {
			if (WtOTPData[ pOtpParams->groupInfo.CheckItemOffset[j]] == pOtpParams->groupInfo.GroupFlag) {
				CAM_ERR(CAM_EEPROM, "part:select group: %d.", j);
				CAM_ERR(CAM_EEPROM, "WtOTPData[i]:%d,GroupFlag:%d",WtOTPData[ pOtpParams->groupInfo.CheckItemOffset[j]],pOtpParams->groupInfo.GroupFlag);
				pOtpParams->groupInfo.SelectGroupNum = j ;
				break;
			} else {
				CAM_ERR(CAM_EEPROM, "WtOTPData[i]:%d,GroupFlag:%d",WtOTPData[ pOtpParams->groupInfo.CheckItemOffset[j]],pOtpParams->groupInfo.GroupFlag);
				continue;
		...
		//can not match group ...
		if( j == GROUPNUMMAX ) {
			CAM_ERR(CAM_EEPROM, "part: %d bad otp data.", j);
			pOtpParams->groupInfo.SelectGroupNum = GROUPNUMMAX; //GROUPNUMMAX is invalid group num.
		}
	}
	return eeprom_result;
}
...
int eeprom_memory_process_otp_data(struct OplusOtpCheckInfo *pOtpParams,uint8_t *memptr){
	...
	for(i=0; i<CHECKNUMMAX; i++){
		SelectGroupTemp = pOtpParams->groupInfo.SelectGroupNum;
		GroupOffset = pOtpParams->ItemInfo[i][SelectGroupTemp].Offset;
		GroupLenth = pOtpParams->ItemInfo[i][SelectGroupTemp].Length;
		CAM_ERR(CAM_EEPROM, "SelectGroupTemp:%d, GroupOffset:%d, GroupLenth :%d", SelectGroupTemp, GroupOffset, GroupLenth);
		if(1 == pOtpParams->ItemInfo[i][SelectGroupTemp].IsAvailable){
			CAM_ERR(CAM_EEPROM, " eeprom_memory_process_otp_data copy data");
...
int eeprom_memory_map_read_data(uint32_t slave_addr,struct cam_eeprom_memory_map_t emap,
	...
	struct OplusOtpCheckInfo *pWtOtpCheckInfo = NULL;
	...
	if (pWtOtpCheckInfo != NULL) {
		eeprom_result = eeprom_memory_select_group(pWtOtpCheckInfo);
	}else {
		CAM_ERR(CAM_EEPROM, "pWtOtpCheckInfo is NULL.");
		return -1;
	}
	if (eeprom_result < 0) {
