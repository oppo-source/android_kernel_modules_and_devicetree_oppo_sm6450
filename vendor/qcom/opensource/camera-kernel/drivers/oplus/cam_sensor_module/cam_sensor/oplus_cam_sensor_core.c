...
/* vendor_id id is 180(0xB4),it is Fab2 module if vendor_id <= 56(0x38),it is Fab1(0x34) or Fab3(0x38) module*/
#define IMX615_FAB2_VENDOR_ID 56
#define IMX615_FAB2_SENSOR_ID 56
/* vendor_id > 512(0x0200),it is long module. if vendor_id > 1792(0x0700),it is short module*/
#define S5KJN5_SHORT_SENSOR_ID  1792
#define S5KJN5_SHORT_VENDOR_ID  1792
bool is_AON_current = 0;
bool is_ftm_current_test = false;
int aon_irq_cnt = 0;
struct cam_sensor_i2c_reg_setting sensor_setting;

//get unique sensorid for cloud eeprom data
#define CAMERA_UNIQUEID_LENGTH 12
struct camera_unique_id
{
    unsigned char unique_sensor_id[CAMERA_UNIQUEID_LENGTH]; //value of sensor uniqueid reg
    unsigned int sensor_reg_len;                            //num of sensor uniqueid reg
    unsigned int sensor_reg_step;                            //step of sensor uniqueid reg
    char sensor_name[CAM_SENSOR_NAME_MAX_SIZE];             //sensor name
};
...
int cam_ftm_power_down(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->sensordata->slave_info.sensor_id == 0x586 ||
		...
		s_ctrl->sensordata->slave_info.sensor_id == 0x0709||
		s_ctrl->sensordata->slave_info.sensor_id == 0x1634||
		s_ctrl->sensordata->slave_info.sensor_id == 0x38E5||
		s_ctrl->sensordata->slave_info.sensor_id == 0xd154||
		s_ctrl->sensordata->slave_info.sensor_id == 0x0203||
		s_ctrl->sensordata->slave_info.sensor_id == 0x5044||
		s_ctrl->sensordata->slave_info.sensor_id == 0x32e2||
		s_ctrl->sensordata->slave_info.sensor_id == 0x38ee||
		s_ctrl->sensordata->slave_info.sensor_id == 0x08a8||
		s_ctrl->sensordata->slave_info.sensor_id == 0xb787)
	{
		sensor_setting.reg_setting = sensor_settings.streamoff.reg_setting;
		sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
		sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
...
int cam_ftm_power_up(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	else
	{
		...
		{
			CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			sensor_setting.reg_setting = sensor_init_settings.sc820cs_amberT_setting.reg_setting;
			sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
			sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
			sensor_setting.size = sensor_init_settings.sc820cs_amberT_setting.size;
			sensor_setting.delay = sensor_init_settings.sc820cs_amberT_setting.delay;
			rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
		}
		else if (s_ctrl->sensordata->slave_info.sensor_id == 0x0203)
		{
			CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			sensor_setting.reg_setting = sensor_init_settings.gc02m3_amberN_setting.reg_setting;
			sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
			sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
			sensor_setting.size = sensor_init_settings.gc02m3_amberN_setting.size;
			sensor_setting.delay = sensor_init_settings.gc02m3_amberN_setting.delay;
			rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
		}
		else if (s_ctrl->sensordata->slave_info.sensor_id == 0x38E5)
		{
			CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			sensor_setting.reg_setting = sensor_init_settings.s5kjn5_setting.reg_setting;
			sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
			sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_WORD;
			sensor_setting.size = sensor_init_settings.s5kjn5_setting.size;
			sensor_setting.delay = sensor_init_settings.s5kjn5_setting.delay;
			rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
		}
		else if (s_ctrl->sensordata->slave_info.sensor_id == 0x0615)
		{
			...
			if(rc < 0)
			{
				CAM_ERR(CAM_SENSOR, "FTM GET sensor vendor_id/SensorID error !");
			}
			else
			{
				if(s_ctrl->sensordata->id_info.sensor_id <= IMX615_FAB2_SENSOR_ID && vendor_id <= IMX615_FAB2_VENDOR_ID)
				{
					...
					sensor_setting.addr_type = sensor_settings.imx615_setting.addr_type;
					sensor_setting.data_type = sensor_settings.imx615_setting.data_type;
					sensor_setting.size = sensor_settings.imx615_setting.size;
					sensor_setting.delay = sensor_settings.imx615_setting.delay;
					rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
				}
				else if(s_ctrl->sensordata->id_info.sensor_id > IMX615_FAB2_SENSOR_ID && vendor_id > IMX615_FAB2_VENDOR_ID)
				{
		...
		else if (s_ctrl->sensordata->slave_info.sensor_id == 0x8202)
		{
			CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
		...
		{
			CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			sensor_setting.reg_setting = sensor_settings.imx709_coffeeA_setting.reg_setting;
			sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
			sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
			sensor_setting.size = sensor_settings.imx709_coffeeA_setting.size;
			sensor_setting.delay = sensor_settings.imx709_coffeeA_setting.delay;
			rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
		}
		else if (s_ctrl->sensordata->slave_info.sensor_id == 0x1634)
		{
			CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			sensor_setting.reg_setting = sensor_settings.hi1634q_coffeeA_setting.reg_setting;
			sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
			sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_WORD;
			sensor_setting.size = sensor_settings.hi1634q_coffeeA_setting.size;
			sensor_setting.delay = sensor_settings.hi1634q_coffeeA_setting.delay;
			rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
		}
		else if(s_ctrl->sensordata->slave_info.sensor_id == 0x5044)
		{
			CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			sensor_setting.reg_setting = sensor_init_settings.ov50d40_milkwayS3_setting.reg_setting;
			sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
			sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
			sensor_setting.size = sensor_init_settings.ov50d40_milkwayS3_setting.size;
			sensor_setting.delay = sensor_init_settings.ov50d40_milkwayS3_setting.delay;
			rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
		}
		else if(s_ctrl->sensordata->slave_info.sensor_id == 0x32e2)
		{
			CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			sensor_setting.reg_setting = sensor_init_settings.gc32e2_milkwayS3_setting.reg_setting;
			sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
			sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
			sensor_setting.size = sensor_init_settings.gc32e2_milkwayS3_setting.size;
			sensor_setting.delay = sensor_init_settings.gc32e2_milkwayS3_setting.delay;
			rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
		}
		else if(s_ctrl->sensordata->slave_info.sensor_id == 0x38ee)
		{
			CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			sensor_setting.reg_setting = sensor_init_settings.s5kjns_alphammain_setting.reg_setting;
			sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
			sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_WORD;
			sensor_setting.size = sensor_init_settings.s5kjns_alphammain_setting.size;
			sensor_setting.delay = sensor_init_settings.s5kjns_alphammain_setting.delay;
			rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
		}
		else if(s_ctrl->sensordata->slave_info.sensor_id == 0x08a8)
		{
			CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			sensor_setting.reg_setting = sensor_init_settings.gc08a8_alphamfront_setting.reg_setting;
			sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
			sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
			sensor_setting.size = sensor_init_settings.gc08a8_alphamfront_setting.size;
			sensor_setting.delay = sensor_init_settings.gc08a8_alphamfront_setting.delay;
			rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
		}
		else if(s_ctrl->sensordata->slave_info.sensor_id == 0xb787)
		{
			CAM_ERR(CAM_SENSOR, "FTM sensor setting 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			sensor_setting.reg_setting = sensor_init_settings.gc532hs_whoopassmain_setting.reg_setting;
			sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
			sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
			sensor_setting.size = sensor_init_settings.gc532hs_whoopassmain_setting.size;
			sensor_setting.delay = sensor_init_settings.gc532hs_whoopassmain_setting.delay;
			rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_setting);
		}
		else
		{
	...
	sensor_setting.reg_setting = sensor_init_settings.streamon.reg_setting;
	sensor_setting.addr_type = sensor_init_settings.streamon.addr_type;
	sensor_setting.data_type = sensor_init_settings.streamon.data_type;
	sensor_setting.size = sensor_init_settings.streamon.size;
	sensor_setting.delay = sensor_init_settings.streamon.delay;
	rc = camera_io_dev_write(&(s_ctrl->io_master_info),&sensor_setting);
	if (rc < 0)
	{
		CAM_ERR(CAM_SENSOR, "FTM Failed to write streamon setting");
		goto power_down;
	}
	else
	{
		CAM_ERR(CAM_SENSOR, "FTM successfully to write streamon setting");
	}
...
int cam_sensor_match_id_oem(struct cam_sensor_ctrl_t *s_ctrl,uint32_t chip_id)
{
	...
	if(chip_id == CAM_IMX709_SENSOR_ID){
	...
	else if(chip_id == CAM_IMX615_SENSOR_ID){
		SensorRegWrite(s_ctrl,0x0A02,0x7F);//Specify OTP Page Address for Read
		SensorRegWrite(s_ctrl,0x0A00,0x01);//Turn ON OTP Read Mode
		...
		rc = camera_io_dev_read(
			...
			CAMERA_SENSOR_I2C_TYPE_BYTE);//Read Fab Id
		...
		if(vendor_id > IMX615_FAB2_VENDOR_ID){
			if(s_ctrl->sensordata->id_info.sensor_id > IMX615_FAB2_SENSOR_ID)
			{
				return 0;
			}
			else
		...
		else if(vendor_id <= IMX615_FAB2_VENDOR_ID)
		{
			if(s_ctrl->sensordata->id_info.sensor_id <= IMX615_FAB2_SENSOR_ID)
			{
				return 0;
		...
		if(vendor_id < S5KJN5_SHORT_VENDOR_ID){
			...
			{
				CAM_INFO(CAM_SENSOR, "S5KJN5 use long init setting");
				return 0;
			}
			else
			{
				CAM_INFO(CAM_SENSOR, "S5KJN5 should use long init setting,but vendor_id 0x%x is wrong",vendor_id);
				return -1;
			}
		}
		...
		{	if(s_ctrl->sensordata->id_info.sensor_id >= S5KJN5_SHORT_SENSOR_ID)
			{
				CAM_INFO(CAM_SENSOR, "S5KJN5 use short init setting");
				return 0;
			}
			else
			{
				CAM_INFO(CAM_SENSOR, "S5KJN5 should use short setting,but vendor_id 0x%x is wrong",vendor_id);
				return -1;
			}
		}
	}
	...
	if (rc == 0)
	{
		...
		if(s_ctrl->sensor_initsetting_state == CAM_SENSOR_SETTING_WRITE_INVALID)
		{
			...
			else if(s_ctrl->sensordata->slave_info.sensor_id == 0x0615)
			{
				...
				if(rc < 0)
				{
					CAM_ERR(CAM_SENSOR, " sensor read vendor_id or SensorID error!");
				}
				else
				{
					CAM_ERR(CAM_SENSOR, "IMX615 vendor/fab id expect 0x%x read  0x%x",s_ctrl->sensordata->id_info.sensor_id,vendor_id);
					if(s_ctrl->sensordata->id_info.sensor_id <= IMX615_FAB2_SENSOR_ID && vendor_id <= IMX615_FAB2_VENDOR_ID)
					{
						CAM_INFO(CAM_SENSOR, "Enter imx615 Fab1/3 powerup!");
						sensor_init_setting.reg_setting = sensor_init_settings.imx615_setting.reg_setting;
						sensor_init_setting.addr_type = sensor_init_settings.imx615_setting.addr_type;
						sensor_init_setting.data_type = sensor_init_settings.imx615_setting.data_type;
					...
					{
						CAM_INFO(CAM_SENSOR, "Enter imx615 Fab2 powerup!");
						sensor_init_setting.reg_setting = sensor_init_settings.imx615_setting_fab2.reg_setting;
						sensor_init_setting.addr_type = sensor_init_settings.imx615_setting_fab2.addr_type;
						sensor_init_setting.data_type = sensor_init_settings.imx615_setting_fab2.data_type;
						sensor_init_setting.size = sensor_init_settings.imx615_setting_fab2.size;
						sensor_init_setting.delay = sensor_init_settings.imx615_setting_fab2.delay;
					}
				}
			}
			...
			else if(s_ctrl->sensordata->slave_info.sensor_id == 0x1634)
			{
				if(s_ctrl->power_up_advance == 1)
				{
					sensor_init_setting.reg_setting = sensor_init_settings.hi1634q_coffeeA_setting.reg_setting;
					sensor_init_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
					sensor_init_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
					sensor_init_setting.size = sensor_init_settings.hi1634q_coffeeA_setting.size;
					sensor_init_setting.delay = sensor_init_settings.hi1634q_coffeeA_setting.delay;
					rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_init_setting);
					if(rc < 0)
					{
						CAM_ERR(CAM_SENSOR, "write 1634  setting failed!");
					}
					else
					{
						CAM_INFO(CAM_SENSOR, "write 1634  setting success!");
						s_ctrl->sensor_initsetting_state = CAM_SENSOR_SETTING_WRITE_SUCCESS;
					}
				}
			}
			else if(s_ctrl->sensordata->slave_info.sensor_id == 0x8202)
			{
				if(s_ctrl->power_up_advance == 1)
				{
					sensor_init_setting.reg_setting = sensor_init_settings.imx882_coffeeA_setting.reg_setting;
					sensor_init_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
					sensor_init_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
					sensor_init_setting.size = sensor_init_settings.imx882_coffeeA_setting.size;
					sensor_init_setting.delay = sensor_init_settings.imx882_coffeeA_setting.delay;
					rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_init_setting);
					if(rc < 0)
					{
						CAM_ERR(CAM_SENSOR, "write 882  setting failed!");
					}
					else
					{
						CAM_INFO(CAM_SENSOR, "write 882  setting success!");
						s_ctrl->sensor_initsetting_state = CAM_SENSOR_SETTING_WRITE_SUCCESS;
					}
					if(rc == 0 && s_ctrl->sensor_qsc_setting.enable_qsc_write_in_advance && s_ctrl->sensor_qsc_setting.read_qsc_success)
					{
						...
						if(rc < 0)
						{
							CAM_ERR(CAM_SENSOR, "write 882  qsc failed!");
						}
						else
						{
							CAM_INFO(CAM_SENSOR, "write 882  qsc success!");
				...
				{
					sensor_init_setting.reg_setting = sensor_init_settings.imx882_setting.reg_setting;
					sensor_init_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
					sensor_init_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
					sensor_init_setting.size = sensor_init_settings.imx882_setting.size;
					sensor_init_setting.delay = sensor_init_settings.imx882_setting.delay;
					rc = camera_io_dev_write(&(s_ctrl->io_master_info), &sensor_init_setting);
					if(rc < 0)
					{
						CAM_ERR(CAM_SENSOR, "write 882 setting failed!");
...
int cam_sensor_start(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if(s_ctrl->sensor_power_state == CAM_SENSOR_POWER_OFF)
	{
		if(s_ctrl->sensordata->slave_info.sensor_id == 0x0615)
		{
			...
			if(rc < 0)
			{
				CAM_ERR(CAM_SENSOR, "IMX615 vendor/fab id read fail!");
			}
			else
			{
				CAM_ERR(CAM_SENSOR, "IMX615 vendor/fab id expect 0x%x read  0x%x",s_ctrl->sensordata->id_info.sensor_id,vendor_id);
...
//Get unique sensorid for cloud eeprom
int32_t oplus_cam_read_unique_sensorid(struct cam_sensor_ctrl_t *s_ctrl, uint32_t addr, uint32_t *data,enum camera_sensor_i2c_type addr_type,enum camera_sensor_i2c_type data_type)
{
    int rc = 0;
    struct cam_sensor_cci_client sensor_cci_client;
    memcpy(&sensor_cci_client, s_ctrl->io_master_info.cci_client,
            sizeof(struct cam_sensor_cci_client));
    rc = cam_cci_i2c_read(&sensor_cci_client,
            addr,
            data,
            addr_type,
            data_type);
...
int32_t oplus_cam_prepare_read_unique_sensorid(struct cam_sensor_ctrl_t *s_ctrl,enum camera_sensor_i2c_type addr_type,enum camera_sensor_i2c_type data_type)
{
    int rc = 0;
    struct cam_sensor_i2c_reg_setting unique_setting;
    CAM_INFO(CAM_SENSOR, "--Cloud Eeprom--start prepare read unique sensorId");
    if (s_ctrl->sensordata->slave_info.sensor_id == 0x38ee)
    {
        CAM_ERR(CAM_SENSOR, "read_unique_sensorid sensorId 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
        //initial
        unique_setting.reg_setting = sensor_init_settings.s5kjns_alphammain_setting.reg_setting;
        unique_setting.addr_type = addr_type;
        unique_setting.data_type = data_type;
        unique_setting.size = sensor_init_settings.s5kjns_alphammain_setting.size;
        unique_setting.delay = sensor_init_settings.s5kjns_alphammain_setting.delay;
        rc = camera_io_dev_write(&(s_ctrl->io_master_info), &unique_setting);
        if(rc < 0){
          CAM_ERR(CAM_SENSOR, "--Cloud Eeprom--prepare read initial setting write fail");
        }

        //clk
        unique_setting.reg_setting = sensor_settings.alphammain_unique_id_read_open_setting_0.reg_setting;
        unique_setting.addr_type = addr_type;
        unique_setting.data_type = data_type;
        unique_setting.size = sensor_settings.alphammain_unique_id_read_open_setting_0.size;
        unique_setting.delay = sensor_settings.alphammain_unique_id_read_open_setting_0.delay;
        rc = camera_io_dev_write(&(s_ctrl->io_master_info), &unique_setting);
        if(rc < 0){
          CAM_ERR(CAM_SENSOR, "--Cloud Eeprom--prepare read clk setting write fail");
        }

        unique_setting.reg_setting = sensor_settings.alphammain_unique_id_read_open_setting_1.reg_setting;
        unique_setting.addr_type = addr_type;
        unique_setting.data_type = data_type;
        unique_setting.size = sensor_settings.alphammain_unique_id_read_open_setting_1.size;
        unique_setting.delay = sensor_settings.alphammain_unique_id_read_open_setting_1.delay;
        rc = camera_io_dev_write(&(s_ctrl->io_master_info), &unique_setting);
        if(rc < 0){
          CAM_ERR(CAM_SENSOR, "--Cloud Eeprom--prepare read open setting write fail");
        }
    }else if(s_ctrl->sensordata->slave_info.sensor_id == 0x08a8){
       //initial
       unique_setting.reg_setting = sensor_init_settings.gc08a8_alphamfront_setting.reg_setting;
       unique_setting.addr_type = addr_type;
       unique_setting.data_type = data_type;
       unique_setting.size = sensor_init_settings.gc08a8_alphamfront_setting.size;
       unique_setting.delay = sensor_init_settings.gc08a8_alphamfront_setting.delay;
       rc = camera_io_dev_write(&(s_ctrl->io_master_info), &unique_setting);
       if(rc < 0){
          CAM_ERR(CAM_SENSOR, "--Cloud Eeprom--prepare read init setting write fail");
       }

       //change page
       unique_setting.reg_setting = sensor_settings.alphamfront_unique_id_read_open_setting_0.reg_setting;
       unique_setting.addr_type = addr_type;
       unique_setting.data_type = data_type;
       unique_setting.size = sensor_settings.alphamfront_unique_id_read_open_setting_0.size;
       unique_setting.delay = sensor_settings.alphamfront_unique_id_read_open_setting_0.delay;
        rc = camera_io_dev_write(&(s_ctrl->io_master_info), &unique_setting);
        if(rc < 0){
          CAM_INFO(CAM_SENSOR, "--Cloud Eeprom--prepare read change page setting write fail");
       }
    }
    return rc;
}
...
int32_t oplus_cam_get_unique_sensorid(struct cam_sensor_ctrl_t *s_ctrl, void *arg)
{
    int rc = 0;
    struct cam_control *cmd = (struct cam_control *)arg;
    struct cam_camera_slave_info *slave_info;
    ...
    slave_info = &(s_ctrl->sensordata->slave_info);
    switch (cmd->op_code) {
        case CAM_OEM_GET_UNIQUE_SENSORID : {
            uint32_t sensor_reg_addr = slave_info->sensor_unique_id_reg_addr;
            uint32_t sensor_reg_len = slave_info->sensor_unique_id_reg_len;
            uint32_t sensor_reg_step = slave_info->sensor_unique_id_reg_step;
            uint32_t sensor_reg_value = 0;
            struct camera_unique_id cam_unique_id = {};
            cam_unique_id.sensor_reg_len = sensor_reg_len;
            cam_unique_id.sensor_reg_step = sensor_reg_step;
            memcpy(cam_unique_id.sensor_name, s_ctrl->sensor_name, sizeof(s_ctrl->sensor_name));
           if(s_ctrl->sensordata->slave_info.sensor_id == 0x38ee){
                rc = oplus_cam_prepare_read_unique_sensorid(s_ctrl,CAMERA_SENSOR_I2C_TYPE_WORD,CAMERA_SENSOR_I2C_TYPE_WORD);
                if(rc < 0){
                  CAM_INFO(CAM_SENSOR, "--Cloud Eeprom--prepare read fail");
                }
                for(i = 0; i < sensor_reg_len; i++)
                {
                    rc = oplus_cam_read_unique_sensorid(s_ctrl,sensor_reg_addr,&sensor_reg_value,CAMERA_SENSOR_I2C_TYPE_WORD,CAMERA_SENSOR_I2C_TYPE_BYTE);
                    CAM_ERR(CAM_SENSOR, "--Cloud Eeprom--sensor_reg_addr:%x, sensor_reg_value:%x, rc = %d,i = %d",sensor_reg_addr, sensor_reg_value, rc,i);
                    cam_unique_id.unique_sensor_id[i] = sensor_reg_value;
                    sensor_reg_addr += sensor_reg_step;
                    if(sensor_reg_addr >= slave_info->sensor_unique_id_reg_addr + sensor_reg_len){
                        break;
                    }
                }
           }else if(s_ctrl->sensordata->slave_info.sensor_id == 0x08a8){
                  int rc = 0;
                  struct cam_sensor_i2c_reg_setting unique_setting;
                  rc = oplus_cam_prepare_read_unique_sensorid(s_ctrl,CAMERA_SENSOR_I2C_TYPE_WORD,CAMERA_SENSOR_I2C_TYPE_BYTE);
                  if(rc < 0){
                     CAM_INFO(CAM_SENSOR, "--Cloud Eeprom--prepare read fail");
                  }
                   for(uint32_t i = 0; i < sensor_reg_len;i++){
                       unique_setting.reg_setting = sensor_settings.alphamfront_unique_id_read_open_setting_1.reg_setting;
                       unique_setting.reg_setting[1].reg_data = sensor_reg_addr;
                       unique_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
                       unique_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
                       unique_setting.size = sensor_settings.alphamfront_unique_id_read_open_setting_1.size;
                       unique_setting.delay = sensor_settings.alphamfront_unique_id_read_open_setting_1.delay;
                       rc = camera_io_dev_write(&(s_ctrl->io_master_info), &unique_setting);
                       if(rc < 0){
                            CAM_INFO(CAM_SENSOR, "--Cloud Eeprom--prepare read setting write fail i=%d",i);
                       }
                       rc = oplus_cam_read_unique_sensorid(s_ctrl,0x0a6c,&sensor_reg_value,CAMERA_SENSOR_I2C_TYPE_WORD,CAMERA_SENSOR_I2C_TYPE_BYTE);
                       CAM_INFO(CAM_SENSOR, "--Cloud Eeprom--sensor_reg_addr:%x, sensor_reg_value:%x, rc = %d,i = %d",sensor_reg_addr, sensor_reg_value, rc,i);
                       cam_unique_id.unique_sensor_id[i] = sensor_reg_value;
                       sensor_reg_addr += sensor_reg_step;
                       if(sensor_reg_addr >= slave_info->sensor_unique_id_reg_addr + sensor_reg_len){
                           break;
               ...
               CAM_ERR(CAM_SENSOR,"--Cloud Eeprom--invalid sensor id 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
            }
            CAM_INFO(CAM_SENSOR, "--Cloud Eeprom--%s oplus_cam_get_unique_sensorid, sensor_reg_len:%x",s_ctrl->sensor_name, cam_unique_id.sensor_reg_len);
            if (copy_to_user((void __user *)cmd->handle,&cam_unique_id,
                sizeof(cam_unique_id))) {
                CAM_ERR(CAM_SENSOR,
                "--Cloud Eeprom--copy camera unique sensor id to user fail ");
            }
            break;
        }
        default:
