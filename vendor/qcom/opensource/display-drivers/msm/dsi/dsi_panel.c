...
#ifdef OPLUS_FEATURE_DISPLAY
	{
		if (!strcmp(panel->name, "AC240 P 3 A0004 dsc cmd mode panel")) {
			if (1 == shutdown_flag) {
				DSI_INFO("AC240 P 3 A0004 power_off reset off");
...
#endif /* OPLUS_FEATURE_DISPLAY */
{
	...
	{
		if (oplus_ofp_video_mode_aod_fod_is_enabled() && mode->timing.refresh_rate == 60
		...
		} else if (oplus_ofp_video_mode_30hz_aod_is_enabled()) {
			type = oplus_ofp_video_mode_ramless_lhbm_pressed_icon_backlight_gamma_update(panel, type);
			if(!type) {
				DSI_DEBUG("type is error");
				goto error;
			}
		}
	}
