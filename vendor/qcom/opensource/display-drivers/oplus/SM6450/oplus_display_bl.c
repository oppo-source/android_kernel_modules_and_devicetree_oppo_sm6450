...
void oplus_panel_backlight_demura_send_async_handler(struct work_struct *work)
{
	...
	refresh_rate = display->modes->timing.refresh_rate;
...
void oplus_panel_backlight_demura_dbv_switch(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if ((panel->oplus_panel.bl_demura_mode != bl_demura_last_mode || oplus_last_backlight <= 1)
		&& (panel->power_mode == SDE_MODE_DPMS_ON) && !oplus_ofp_get_hbm_state()) {
...
int oplus_ofp_video_mode_ramless_lhbm_pressed_icon_backlight_gamma_update(void *dsi_panel, enum dsi_cmd_set_type type)
{
	...
	if (bl_level != oplus_last_backlight) {
		 bl_level = oplus_last_backlight;
		 OFP_INFO("last backlight != bl_level,update bl_level. oplus_last_backlight:%d,bl_level:%d\n", oplus_last_backlight, bl_level);
	}
