...
static int oplus_panel_parse_serial_number_info(struct dsi_panel *panel)
{
	...
	if (panel->oplus_panel.serial_number.serial_number_support) {
		...
		if (!regs) {
			...
			panel->oplus_panel.serial_number.serial_number_multi_regs =
				kzalloc((sizeof(u32) * len), GFP_KERNEL);
			if (!panel->oplus_panel.serial_number.serial_number_multi_regs)
				return -EINVAL;
			for (i = 0; i < len; i++) {
				panel->oplus_panel.serial_number.serial_number_multi_regs[i] = regs[i];
			}
			panel->oplus_panel.serial_number.serial_number_conut = len;
		}
		panel->oplus_panel.serial_number.is_multi_reg = utils->read_bool(utils->data,
			"oplus,dsi-serial-number-multi-reg");
		DSI_INFO("oplus,dsi-serial-number-multi-reg: %s", panel->oplus_panel.serial_number.is_multi_reg ? "true" : "false");
	}
...
int oplus_panel_parse_features_config(struct dsi_panel *panel)
{
	...
	OPLUS_DSI_INFO("oplus,dsi-vid-timming-switch_enable: %s\n",
