		...
		struct dsi_panel *panel, u8 cmd, void *rbuf,  size_t len)
{
	...
	if ((!strcmp(panel->name, "AA570 P 1 A0017 vid mode panel")||
	    !strcmp(panel->name, "AC238 P 1 A0017 vid mode panel") ||
	    !strcmp(panel->name, "AC292 P 7 A0025 vid mode panel") ||
	    !strcmp(panel->name, "AC292 P 3 A0025 vid mode panel") ||
	    !strcmp(panel->name, "AC334 P b A0029 vid mode panel") ||
	    !strcmp(panel->name, "AC334 P 7 A0025 vid mode panel")) &&
...
static ssize_t oplus_display_get_panel_btb_sn(struct kobject *obj,
		struct kobj_attribute *attr, char *buf)
{
	int ret = 0;
	unsigned char btb_buffer[64] = {};
	...
	if (display->panel->power_mode != SDE_MODE_DPMS_ON) {
		ret = sysfs_emit(buf, "Get panel btb sn : failed for panel off");
		OPLUS_DSI_INFO("display panel in off status\n");
		return ret;
	}

	if (!display->panel->panel_initialized) {
		ret = sysfs_emit(buf, "Get panel btb sn : failed for panel not init");
		OPLUS_DSI_ERR("panel initialized = false\n");
		return ret;
	}

	if (!display->panel->oplus_panel.btb_sn.btb_sn_support) {
		ret = sysfs_emit(buf, "Get panel btb sn : failed for not support");
	...
	for (i = 0; i < 5; i++) {
		...
		} else if (!display->panel->oplus_panel.btb_sn.is_reg_lock) {
		...
		ret = dsi_display_read_panel_reg(display, display->panel->oplus_panel.btb_sn.btb_sn_reg,
				btb_buffer, display->panel->oplus_panel.btb_sn.btb_sn_conut);
		mutex_unlock(&display->display_lock);
		...
		ret = sysfs_emit(buf, "Get panel btb sn: %s", btb_buffer + display->panel->oplus_panel.btb_sn.btb_sn_index);
		break;
	}
		...
		bool force_disable)
{
	...
	if (oplus_datadimming_v2_need_flush) {
		if (oplus_datadimming_v2_need_sync
			...
			if (!ret) {
				OPLUS_DSI_ERR("crtc wait_event_timeout \n");
			}
		}
		if (display->config.panel_mode == DSI_OP_VIDEO_MODE) {
			panel->oplus_panel.skip_mipi_last_cmd = true;
		}

		dsi_panel_seed_mode_unlock(panel, seed_mode);

		if (display->config.panel_mode == DSI_OP_VIDEO_MODE) {
			panel->oplus_panel.skip_mipi_last_cmd = false;
		}
				  ...
				  bool force_disable)
{
	...
	if (!force_disable && oplus_dimlayer_bl_enable_v2_real &&
		...
		if (!oplus_seed_backlight) {
			OPLUS_DSI_ERR("Enter DC backlight v2\n");
		...
		OPLUS_DSI_ERR("Exit DC backlight v2\n");
...
static OPLUS_ATTR(panel_btb_sn, S_IRUGO | S_IWUSR,
		oplus_display_get_panel_btb_sn, NULL);
