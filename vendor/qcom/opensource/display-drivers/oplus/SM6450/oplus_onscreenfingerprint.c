...
int oplus_ofp_parse_dtsi_config(void *dsi_display_mode, void *dsi_parser_utils)
{
	...
	*/
	...
	/* indicates how many frames does hbm on cmds take effect When the frame rate switches from 60 to 120 */
	rc = utils->read_u32(utils->data, "oplus,ofp-hbm-on-period-in-timing-switch", &data);
	if (rc) {
		OFP_DEBUG("failed to parse oplus,ofp-hbm-on-period-in-timing-switch\n");
		priv_info->oplus_ofp_hbm_on_period_in_timing_swtch = 0;
	} else {
		priv_info->oplus_ofp_hbm_on_period_in_timing_swtch = data;
	}
	OFP_DEBUG("oplus_ofp_hbm_on_period_in_timing_swtch:%u\n", priv_info->oplus_ofp_hbm_on_period_in_timing_swtch);
...
static int oplus_ofp_hbm_wait_handle(void *sde_connector, bool hbm_en)
{
	...
	unsigned int delay_us = 0;
	unsigned int hbm_on_period_in_timing_swtch = 0;
	struct sde_connector *c_conn = sde_connector;
	struct dsi_display *display = NULL;
	struct oplus_ofp_params *p_oplus_ofp_params = oplus_ofp_get_params(oplus_ofp_display_id);
	...
	us_per_frame = 1000000/refresh_rate;
	hbm_on_period_in_timing_swtch = display->panel->cur_mode->priv_info->oplus_ofp_hbm_on_period_in_timing_swtch;
		...
		if (p_oplus_ofp_params->aod_unlocking) {
			if (display->panel->cur_mode->priv_info->oplus_ofp_aod_off_insert_black_frame) {
				...
				if (need_aod_off_hbm_on_delay) {
					...
					if (refresh_rate != display->panel->oplus_panel.last_refresh_rate) {
						te_count = hbm_on_period_in_timing_swtch;
					} else {
						te_count = 1;
					}
					delay_us = (us_per_frame >> 1) + 700;
